...
// 回显数据
  ...
  queryAgencyInfo: function() {
    //获取行业类别
    var mchtMngScope = wx.getStorageSync("mchtMngScope");
    if (util.strIsNotEmpty(mchtMngScope)) {
      this.setData({
        mchtMngScope: mchtMngScope,
      });
    }
  },
  ...
  queryMomMerchantName: function() {
    var that = this;
    wx.request({
      url: "https://ydsd.bsb.com.cn/ifsp-gateway/agent/queryMomMerchantName.do", //本地服务器地址
      data: {
      ...
      success: function(res) {
         mchtMngNoList = res.data.empList;
  ...
  checkFiled: function (e){
    ...
    if (util.strIsEmpty(wx.getStorageSync("mchtMngScope")) || util.strIsEmpty(wx.getStorageSync("mchtMngScope")[0].custNo)) {
      util.showToast('请选择行业类别！');
      return false;
    }
    var custNo = wx.getStorageSync("mchtMngScope")[0].custNo;
